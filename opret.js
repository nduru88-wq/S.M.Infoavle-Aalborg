...
function saetLoginVenter(erVenter) {
  ...
  if (knap) {
    ...
    knap.classList.toggle("logger-ind", erVenter);
    knap.textContent = erVenter ? "Logger ind..." : "Log ind";
  }

  if (status) {
    status.classList.toggle("logger-ind", erVenter);
    status.innerHTML = erVenter
      ? '<span class="login-spinner" aria-hidden="true"></span><span>Logger ind...</span>'
      : "";
  }
}
