...
function logInd() {
  ...
  if (knap) {
    knap.disabled = true;
    knap.classList.add("logger-ind");
    knap.textContent = "Logger ind...";
  }

  if (status) {
    status.className = "logger-ind";
    status.innerHTML = '<span class="login-spinner" aria-hidden="true"></span><span>Logger ind...</span>';
  }
  ...
  apiKald({
    ...
    .catch(function(err) {
      if (knap) {
        knap.disabled = false;
        knap.classList.remove("logger-ind");
        knap.textContent = "Log ind";
      }

      if (status) {
        status.className = "";
        status.textContent = err.message || "Forkert kode";
      }
    });
}
