...
function toggleGentagMenu() {
  const menu = $("gentagMenu");
  const knap = $("gentagKnap");
  if (!menu || !knap) return;
  const aaben = menu.style.display === "block";
  menu.style.display = aaben ? "none" : "block";
  knap.setAttribute("aria-expanded", aaben ? "false" : "true");
}

function lukGentagMenu() {
  if ($("gentagMenu")) $("gentagMenu").style.display = "none";
  if ($("gentagKnap")) $("gentagKnap").setAttribute("aria-expanded", "false");
}

function opdaterGentagKnap() {
  const knap = $("gentagKnap");
  if (!knap) return;
  const tekster = {
    ingen: "Gentag aktivitet ▾",
    dagligt: "Gentag: Dagligt ▾",
    ugentligt: "Gentag: Ugentligt ▾",
    "14dage": "Gentag: Hver 14. dag ▾"
  };
  knap.textContent = tekster[valgtGentagelse] || tekster.ingen;
}

function vaelgGentagelseMenu(type) {
  valgtGentagelse = type || "ingen";
...
function vaelgGentagelse(type) {
  ...
  const hver14Dag = $("gentag14Dage");

  valgtGentagelse = "ingen";

  if (type === "dagligt" && dagligt && dagligt.checked) {
    if (ugentligt) ugentligt.checked = false;
    if (hver14Dag) hver14Dag.checked = false;
    valgtGentagelse = "dagligt";
  }

  if (type === "ugentligt" && ugentligt && ugentligt.checked) {
    if (dagligt) dagligt.checked = false;
    if (hver14Dag) hver14Dag.checked = false;
    valgtGentagelse = "ugentligt";
  }

  if (type === "14dage" && hver14Dag && hver14Dag.checked) {
    if (dagligt) dagligt.checked = false;
    if (ugentligt) ugentligt.checked = false;
...
function toggleHeleDagen() {
  ...
  setDisabledMedOpacity("startTimer", hele);
  setDisabledMedOpacity("startMinutter", hele);
...
function opdaterHeleDagenEfterAktivitet() {
  ...
  setDisabledMedOpacity("startTimer", skalVaereHeldag);
  setDisabledMedOpacity("startMinutter", skalVaereHeldag);
...
function indlaesAktivitetTilRedigering(id) {
  ...
  apiKald({
    ...
    .then(function(a) {
      ...
      if ($("opretTitel")) $("opretTitel").textContent = "REDIGER AKTIVITET";
      if ($("sendTilTavleKnap")) {
      ...
      if (a.tidspunkt && String(a.tidspunkt).includes(":")) {
        const startDele = String(a.tidspunkt).split(":");
        setVal("startTimer", startDele[0]);
        setVal("startMinutter", startDele[1]);
      }
