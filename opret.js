...
function opdaterEfterAktivitet() {
  ...
  const erMobil = window.matchMedia("(max-width: 600px)").matches;

  if (noteBox) {
    if (erMobil) {
      /*
        På mobil beholder Note-feltet altid sin plads.
        Dermed ændrer formularens højde sig ikke, når man vælger aktivitet.
      */
      noteBox.style.display = "block";
      noteBox.style.visibility = visNote ? "visible" : "hidden";
      noteBox.style.pointerEvents = visNote ? "auto" : "none";
    } else {
...
function sendTilTavle() {
  ...
  /* Normal gemme-feedback vises nu direkte i Send-knappen. */
  setHtml("status", "");

  if (sendBtn) {
    sendBtn.disabled = true;
    sendBtn.classList.remove("gemt");
    sendBtn.classList.add("gemmer");
    sendBtn.textContent = "Gemmer...";
  }
  ...
  apiKald(params)
    .then(function() {
      ...
      sendBtn.classList.remove("gemmer");
      sendBtn.classList.add("gemt");
      sendBtn.textContent = redigerSomEnkeltForekomst
        ? "Denne dags aktivitet er gemt"
        : (AKTIVITET_ID ? "Aktiviteten er opdateret" : "Aktiviteten er gemt");

      window.setTimeout(function() {
        sendBtn.classList.remove("gemt");
        sendBtn.disabled = false;
        sendBtn.textContent = "Send til tavle";
      }, 3000);
    })
    .catch(function(err) {
      if (sendBtn) {
        sendBtn.classList.remove("gemmer", "gemt");
        sendBtn.disabled = false;
        sendBtn.textContent = "Send til tavle";
      }
