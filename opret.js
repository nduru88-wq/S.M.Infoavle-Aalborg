...
function goerHeleDatofeltetKlikbart() {
  const datoFelt = $("dato");
  if (!datoFelt) return;

  datoFelt.style.cursor = "pointer";

  datoFelt.addEventListener("click", function() {
    if (typeof datoFelt.showPicker === "function") {
      try {
        datoFelt.showPicker();
      } catch (fejl) {
        // Browseren åbner stadig kalenderen normalt via kalenderikonet.
      }
    }
  });
}

function saetStandardDato() {
  const datoFelt = $("dato");
  if (!datoFelt || datoFelt.value) return;
