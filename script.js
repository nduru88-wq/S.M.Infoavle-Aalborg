...
function visSlideshowBillede() {
  var imgA = $("slideshowImgA");
  var imgB = $("slideshowImgB");

  if (!imgA || !imgB || slideshowBilleder.length === 0) return;

  var nytBillede = slideshowBilleder[slideshowIndex];
  var synlig = imgA.classList.contains("slideshow-synlig") ? imgA : imgB;
  var naeste = synlig === imgA ? imgB : imgA;

  /* Første billede: vis det med det samme uden overgang fra baggrunden. */
  if (!imgA.getAttribute("src") && !imgB.getAttribute("src")) {
    imgA.src = nytBillede;
    imgA.classList.add("slideshow-synlig");
    imgB.classList.remove("slideshow-synlig");
    return;
  }

  /* Forudindlæs næste billede. Det gamle bliver stående, indtil det nye er klar. */
  var preload = new Image();

  preload.onload = function() {
    naeste.src = nytBillede;

    requestAnimationFrame(function() {
      requestAnimationFrame(function() {
        naeste.classList.add("slideshow-synlig");
        synlig.classList.remove("slideshow-synlig");
      });
    });
  };

  preload.onerror = function() {
    /* Ved billedfejl beholdes det gamle billede i stedet for at blinke. */
    console.log("Kunne ikke indlæse slideshow-billede:", nytBillede);
  };

  preload.src = nytBillede;
}
