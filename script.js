  ...
  return String(a.tidspunkt || "").localeCompare(String(b.tidspunkt || ""));
    ...
    if (!dagens.length) {
      ...
      dagens.forEach(function(a) {
        // Gem den konkrete dato kortet vises på. Det bruges bl.a. til at
        // lade noter på gentagelser gælde kun for første forekomst.
...
function lavEventNote(a) {
  ...
  // Ved gentagne grundaktiviteter vises noten kun på startdatoen.
  // En særskilt/redigeret forekomst (parentId) beholder altid sin egen note.
  ...
  if (erGentaget && !a.parentId) {
    var visningsDato = a._visningsDato || "";
    if (visningsDato && visningsDato !== String(a.dato)) {
      note = "";
    }
  }
