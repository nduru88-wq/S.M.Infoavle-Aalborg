...
if (erFoedselsdag) {
  ...
  "<div class='event-main-content'>" +
    "<div class='event-title'><span class='event-title-name'>" + titel + "</span>" + (tid ? "<span class='event-title-time'>" + tid + "</span>" : "") + "</div>" +
    lavEventNote(a) +
  "</div>" +
...
"</div>";
  ...
  // Når tidspunktet hopper ned på en ny linje, fjernes den første bindestreg.
  requestAnimationFrame(function() {
    opdaterTidBindestreg(div);
  });

  return div;
}

function opdaterTidBindestreg(eventEl) {
  if (!eventEl) return;

  var titelEl = eventEl.querySelector(".event-title");
  var navnEl = eventEl.querySelector(".event-title-name");
  var tidEl = eventEl.querySelector(".event-title-time");

  if (!titelEl || !navnEl || !tidEl) return;

  titelEl.classList.toggle(
    "tid-paa-ny-linje",
    tidEl.offsetTop > navnEl.offsetTop + 2
  );
}

function opdaterAlleTidBindestreger() {
  document.querySelectorAll(".event").forEach(opdaterTidBindestreg);
}

window.addEventListener("resize", function() {
  requestAnimationFrame(opdaterAlleTidBindestreger);
});
