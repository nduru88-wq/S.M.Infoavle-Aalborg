...
function varighedTilMinutter(varighed) {
  ...
  /* Ældre timer.minutter-format: 0.15 / 0.30 / 0.45 / 1.15 osv. */
  var gammel = tekst.match(/^(\d+)\.(00|15|30|45)$/);

  if (gammel) {
    return parseInt(gammel[1], 10) * 60 + parseInt(gammel[2], 10);
  }

  /* Ny format: decimaltimer. Afrund til hele minutter for at undgå flydende decimaler. */
  var timer = Number(tekst);

  if (!isFinite(timer)) return NaN;
