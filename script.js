...
function visUge() {
  ...
  var harTidsrum =
    ...
    !isNaN(varighedTilMinutter(x.varighed));
...
function tidspunktErNu(a) {
  ...
  var varighedMin = varighedTilMinutter(a.varighed);
  if (isNaN(varighedMin)) return false;
  var slutMin = startMin + varighedMin;
...
function kaldSletApi(action, params, fejltekst, eventElement) {
  ...
  window[callbackName] = function(data) {
    try {
      ...
      /* HURTIG SLETNING 16-08-2026:
         Når Apps Script har bekræftet sletningen, fjernes den valgte
         aktivitet straks fra den viste tavle. Den fulde synkronisering
         hentes bagefter uden at holde brugeren tilbage. */
...
/***** VARIGHED – understøtter både nye minutter (fx "45m") og gamle decimaltimer (fx 0.75) *****/

function varighedTilMinutter(varighed) {
  if (varighed === null || varighed === undefined || varighed === "") return NaN;
  if (varighed === "Hele dagen") return NaN;

  var tekst = String(varighed).trim().replace(",", ".");

  /* Nyt entydigt format fra opret.js: fx 45m eller 105m */
  if (/^\d+(?:\.\d+)?m$/i.test(tekst)) {
    return Number(tekst.slice(0, -1));
  }

  /* Bagudkompatibilitet: gamle værdier er gemt som antal timer. */
  var timer = Number(tekst);
  if (isNaN(timer)) return NaN;
