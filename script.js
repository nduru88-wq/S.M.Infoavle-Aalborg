var API_URL = "https://script.google.com/macros/s/AKfycbx71tVLXQAQDpAlfHeaIe2Y-cH3zcR5oGFqNt1ChIrP9PmkGwYyz_4FLTa060rJX-aJ/exec";
...
function visUge() {
  ...
  var harTidsrum =
    ...
    isFinite(varighedTilMinutter(x.varighed));
...
return String(a.tidspunkt || "").localeCompare(String(b.tidspunkt || ""));
    ...
    /* En konkret engangsaktivitet på denne dato overtager den tilsvarende
       gentagne grundaktivitet. Det bruges til unikke noter pr. forekomst. */
    dagens = dagens.filter(function(a) {
      var gentagelse = String(a.gentagelse || "ingen").toLowerCase();

      if (gentagelse !== "ugentligt" && gentagelse !== "14dage") {
        return true;
      }

      return !dagens.some(function(b) {
        var bGentagelse = String(b.gentagelse || "ingen").toLowerCase();

        if (bGentagelse !== "ingen" && bGentagelse !== "") {
          return false;
        }

        if (
          b.parentId &&
          String(b.parentId) === String(a.id) &&
          String(b.dato || "") === formatDatoInput(dato)
        ) {
          return true;
        }
        ...
        return (
          ...
          b.id !== a.id &&
          String(b.dato || "") === formatDatoInput(dato) &&
          String(b.person || "") === String(a.person || "") &&
          String(b.aktivitet || "") === String(a.aktivitet || "") &&
          String(b.tidspunkt || "") === String(a.tidspunkt || "")
        );
      });
    });
    ...
    if (!dagens.length) {
      ...
      dagens.forEach(function(a) {
        dagScroll.appendChild(lavEventElement(a, dato));
      });
    }
...
function lavEventElement(a, visningsDato) {
...
if (erFoedselsdag) {
  ...
  "<div class='event-main-content'>" +
    ...
    lavEventDetalje(a, tid, visningsDato) +
  "</div>" +
  "<div class='event-activity-icon'>" + venstreIkon + "</div>" +
    
 "<div class='event-actions'>" +
  "<button class='delete-btn' onclick=\"event.stopPropagation(); sletAktivitetFraTavle('" + id + "', '" + escapeHtml(formatDatoInput(visningsDato || lavDatoFraInput(a.dato))) + "', '" + escapeHtml(String(a.gentagelse || "ingen")) + "', '" + escapeHtml(String(a.parentId || "")) + "', this.closest('.event'))\">Slet</button>" +
  "<button class='edit-btn' onclick=\"event.stopPropagation(); aabnRedigerSide('" + id + "', '" + escapeHtml(formatDatoInput(visningsDato || lavDatoFraInput(a.dato))) + "')\">Ret</button>" +
"</div>";
...
function lavEventDetalje(a, tid, visningsDato) {
  var note = String(a.note || "").trim();
  var gentagelse = String(a.gentagelse || "ingen").toLowerCase();

  /* Grundaktivitetens note gælder kun startdatoen. */
  if (
    (gentagelse === "ugentligt" || gentagelse === "14dage") &&
    visningsDato &&
    formatDatoInput(visningsDato) !== String(a.dato || "")
  ) {
    note = "";
  }
  var dele = [];
...
function tidspunktErNu(a) {
  ...
  var slutMin = startMin + varighedTilMinutter(a.varighed);
...
function aktivitetPasserTilDato(a, dato) {
  ...
  var gentagelse = String(a.gentagelse || "ingen").toLowerCase();
  var testDatoTekst = formatDatoInput(test);
  var gentagTil = String(a.gentagTil || "");
  var udeladteDatoer = String(a.udeladDatoer || "")
    .split(",")
    .map(function(x) { return x.trim(); })
    .filter(Boolean);

  if (gentagTil && testDatoTekst > gentagTil) {
    return false;
  }

  if (udeladteDatoer.indexOf(testDatoTekst) > -1) {
    return false;
  }
  ...
  if (gentagelse === "14dage") {
    ...
    var forskelMs = test.getTime() - start.getTime();
    var forskelDage = Math.round(forskelMs / 86400000);
...
function visAfventerSletning(eventElement) {
  if (!eventElement) return;
  eventElement.classList.remove("vis-menu");
  eventElement.classList.add("afventer-sletning");
  var eksisterende = eventElement.querySelector(".afventer-sletning-overlay");
  ...
  overlay.innerHTML =
    ...
    "<span>Afventer sletning...</span>";
  eventElement.appendChild(overlay);
}

function fjernAfventerSletning(eventElement) {
  if (!eventElement) return;
  eventElement.classList.remove("afventer-sletning");
  var overlay = eventElement.querySelector(".afventer-sletning-overlay");
  if (overlay) overlay.remove();
}

function kaldSletApi(action, params, fejltekst, eventElement) {
  var callbackName = "smSletCallback_" + Date.now() + "_" + Math.floor(Math.random() * 100000);
  ...
  try {
    ...
    fjernAfventerSletning(eventElement);
    alert("Fejl i forbindelsen til Apps Script: " + (err.message || err));
    return;
  }
  ...
  window[callbackName] = function(data) {
    try {
      if (data && data.ok === false) {
        fjernAfventerSletning(eventElement);
        alert(data.message || fejltekst || "Kunne ikke udføre handlingen.");
        return;
      }
...
var gentagSletValg = null;

function lukGentagSletDialog() {
  var modal = $("gentagSletModal");
  if (modal) {
    modal.remove();
  }
...
function visGentagSletDialog(id, dato, eventElement) {
  lukGentagSletDialog();
  ...
  var overlay = document.createElement("div");
  overlay.id = "gentagSletModal";
  overlay.className = "gentag-slet-overlay";

  overlay.innerHTML =
    "<div class='gentag-slet-box' role='dialog' aria-modal='true' aria-labelledby='gentagSletTitel'>" +
      "<div id='gentagSletTitel' class='gentag-slet-titel'>Slet gentaget aktivitet</div>" +
      "<div class='gentag-slet-tekst'>Hvad vil du slette?</div>" +
      "<button type='button' class='gentag-slet-knap kun-denne'>Kun denne dag</button>" +
      "<button type='button' class='gentag-slet-knap fremtid'>Denne og alle fremtidige</button>" +
      "<button type='button' class='gentag-slet-knap annuller'>Annuller</button>" +
    "</div>";

  document.body.appendChild(overlay);

  overlay.querySelector(".kun-denne").addEventListener("click", function() {
    var valg = gentagSletValg;
    lukGentagSletDialog();

    visAfventerSletning(valg.eventElement);

    kaldSletApi(
      "sletForekomst",
      { id: valg.id, dato: valg.dato },
      "Kunne ikke slette denne forekomst.",
      valg.eventElement
    );
  });

  overlay.querySelector(".fremtid").addEventListener("click", function() {
    var valg = gentagSletValg;
    lukGentagSletDialog();
    ...
    kaldSletApi(
      "stopGentagelseFra",
      { id: valg.id, dato: valg.dato },
      "Kunne ikke stoppe gentagelsen.",
      valg.eventElement
    );
  });

  overlay.querySelector(".annuller").addEventListener("click", lukGentagSletDialog);
...
function sletAktivitetFraTavle(id, dato, gentagelse, parentId, eventElement) {
  if (!id) {
    alert("Mangler ID på aktiviteten.");
    return;
  }

  var type = String(gentagelse || "ingen").toLowerCase();
  var erGentaget =
    type === "ugentligt" ||
    type === "14dage" ||
    type === "dagligt" ||
  ...
  if (erGentaget) {
    visGentagSletDialog(id, dato, eventElement);
    return;
  }

  if (!confirm("Vil du slette aktiviteten?")) return;

  visAfventerSletning(eventElement);

  kaldSletApi(
    "sletAktivitet",
    { id: id },
    "Kunne ikke slette aktiviteten.",
    eventElement
  );
}
...
/*
  Varighed har gennem tiden kunnet ligge i to formater:
  - Ny format: decimaltimer, fx 0.75 = 45 min og 1.25 = 1 t 15 min
  - Ældre format: timer.minutter, fx 0.45 = 45 min og 1.15 = 1 t 15 min

  Denne funktion understøtter begge dele og returnerer altid HELE minutter.
  Det forhindrer skæve tider som fx 11:09.
*/
function varighedTilMinutter(varighed) {
  if (varighed === null || varighed === undefined || varighed === "") return NaN;

  var tekst = String(varighed).trim().replace(",", ".");

  /* Nyt entydigt format fra opret-siden: fx "45m" eller "105m". */
  var minutMatch = tekst.match(/^(\d+)m$/i);
  if (minutMatch) {
    return parseInt(minutMatch[1], 10);
  }

  /* Tidligere versioner gemte decimaltimer, fx 0.75 = 45 min. */
  var tal = Number(tekst);
  if (!isFinite(tal)) return NaN;

  var decimalMinutter = Math.round(tal * 60);
  if (decimalMinutter % 15 === 0) {
    return decimalMinutter;
  }

  /* Meget gamle timer.minutter-værdier. */
  var gammel = tekst.match(/^(\d+)\.(15|30|45)$/);
  if (gammel) {
    return parseInt(gammel[1], 10) * 60 + parseInt(gammel[2], 10);
  }
...
function beregnSlutTid(startTid, varighed) {
  ...
  var ekstraMinutter = varighedTilMinutter(varighed);

  if (!isFinite(ekstraMinutter)) return "";

  var startTimer = parseInt(dele[0], 10);
  var startMinutter = parseInt(dele[1], 10);

  if (isNaN(startTimer) || isNaN(startMinutter)) return "";

  var samletMinutter = startTimer * 60 + startMinutter + ekstraMinutter;
  var slutTimer = Math.floor(samletMinutter / 60) % 24;
  var slutMinutter = samletMinutter % 60;

  return pad(slutTimer) + ":" + pad(slutMinutter);
}
...
function aabnRedigerSide(id, dato) {
  if (!id) {
    alert("Mangler ID på aktiviteten.");
    return;
  }

  var url = "opret.html?id=" + encodeURIComponent(id);

  if (dato) {
    url += "&dato=" + encodeURIComponent(dato);
  }
...
/***** SLIDESHOW – BILLEDER, GIF OG VIDEO *****/

var slideshowIndex = 0;
var slideshowBilleder = [];
var slideshowTimer = null;
var slideshowFejlTimer = null;
...
function slideshowFiltype(navn) {
  var tekst = String(navn || "").toLowerCase();

  if (/\.(mp4|mov)$/.test(tekst)) return "video";
  if (/\.(png|jpg|jpeg|webp|gif)$/.test(tekst)) return "billede";

  return "";
}

function normaliserSlideshowListe(liste) {
  if (!Array.isArray(liste)) return [];

  return liste.map(function(fil) {
    if (typeof fil === "string") {
      return {
        url: fil,
        navn: fil.split("/").pop() || "",
        type: slideshowFiltype(fil) || "billede"
      };
    }

    if (fil && typeof fil === "object" && fil.url) {
      return {
        url: fil.url,
        navn: fil.navn || fil.url.split("/").pop() || "",
        type: fil.type || slideshowFiltype(fil.navn || fil.url) || "billede"
      };
    }
  ...
  fetch(SLIDESHOW_API + "?t=" + Date.now(), {
    ...
    .then(function(files) {
      if (!Array.isArray(files)) {
        throw new Error("GitHub returnerede ikke en filliste");
      }

      var nyeFiler = files
        .filter(function(file) {
          return file.type === "file" && slideshowFiltype(file.name);
        })
        .map(function(file) {
          return {
            url: file.download_url,
            navn: file.name,
            type: slideshowFiltype(file.name)
          };
        });

      if (nyeFiler.length === 0) {
        throw new Error("Ingen slideshow-filer fundet");
      }

      slideshowBilleder = nyeFiler;
    ...
    .catch(function(error) {
      console.log("Kunne ikke hente slideshow-filer:", error);

      var gemteBilleder = localStorage.getItem("sm_slideshow_billeder");
      if (!gemteBilleder) return;

      try {
        slideshowBilleder =
          normaliserSlideshowListe(JSON.parse(gemteBilleder));
        ...
        console.log("Kunne ikke bruge gemte slideshow-filer:", fejl);
      }
    });
}

function rydSlideshowTimer() {
  if (slideshowTimer) {
    clearTimeout(slideshowTimer);
    slideshowTimer = null;
  }

  if (slideshowFejlTimer) {
    clearTimeout(slideshowFejlTimer);
    slideshowFejlTimer = null;
  }
}

function naesteSlideshowFil() {
  if (slideshowBilleder.length === 0) return;
...
function skjulSlideshowMedier(skjulBilleder) {
  var billeder = [
    $("slideshowImg"),
    $("slideshowImgNext"),
    $("mobilSlideshowImg"),
    $("mobilSlideshowImgNext")
  ];

  var video = $("slideshowVideo");
  var mobilVideo = $("mobilSlideshowVideo");
  ...
  [video, mobilVideo].forEach(function(v) {
    if (!v) return;
    try { v.pause(); } catch (e) {}
...
function hentGifVarighed(url) {
  return fetch(url, { cache: "no-store" })
    .then(function(response) {
      if (!response.ok) {
        throw new Error("Kunne ikke hente GIF");
      }
    ...
    .then(function(buffer) {
      ...
      if (
        b.length < 13 ||
        String.fromCharCode(b[0], b[1], b[2]) !== "GIF"
      ) {
        throw new Error("Ikke en gyldig GIF");
      }
      ...
      /* Global Color Table */
      if (packed & 0x80) {
        var gctStorrelse = 3 * Math.pow(2, (packed & 0x07) + 1);
        pos += gctStorrelse;
      }

      var totalHundrededele = 0;

      function springSubBlocksOver() {
        while (pos < b.length) {
          var stoerrelse = b[pos++];
          if (stoerrelse === 0) break;
          pos += stoerrelse;
        }
      }
      ...
      while (pos < b.length) {
        ...
        if (blok === 0x21) {
          ...
          if (label === 0xF9) {
            var blokStoerrelse = b[pos++];

            if (blokStoerrelse === 4 && pos + 4 <= b.length) {
              pos++; /* packed field */

              var delay = b[pos] | (b[pos + 1] << 8);
              pos += 2;

              /* Nogle GIF'er bruger 0; brug 10 hundrededele som praktisk minimum. */
              if (delay <= 0) delay = 10;

              totalHundrededele += delay;
            ...
            /* Application, comment, plain text osv. */
            springSubBlocksOver();
          }
        ...
        if (blok === 0x2C) {
          ...
          var imagePacked = b[pos + 8];
          pos += 9;

          /* Local Color Table */
          if (imagePacked & 0x80) {
            var lctStorrelse =
              3 * Math.pow(2, (imagePacked & 0x07) + 1);
            pos += lctStorrelse;
          }
        ...
        /* Ukendt blok – stop sikkert */
        break;
      }

      if (totalHundrededele <= 0) {
        throw new Error("GIF-varighed kunne ikke aflæses");
      }

      return totalHundrededele * 10;
    });
}


function crossfadeSlideshowBillede(fil) {
  var pcA = $("slideshowImg");
  var pcB = $("slideshowImgNext");
  var mobA = $("mobilSlideshowImg");
  var mobB = $("mobilSlideshowImgNext");

  function fadePar(a, b) {
    if (!a) return;
    ...
    var aktiv = a.classList.contains("aktiv") ? a :
                (b.classList.contains("aktiv") ? b : a);
    var naeste = aktiv === a ? b : a;

    naeste.onload = function() {
      naeste.style.display = "block";
      naeste.classList.add("aktiv");
...
function visSlideshowBillede() {
  ...
  var fil = slideshowBilleder[slideshowIndex];
  if (typeof fil === "string") {
    fil = normaliserSlideshowListe([fil])[0];
  }
  ...
  skjulSlideshowMedier(fil.type === "video");
  ...
  if (fil.type === "video") {
    ...
    if (video) {
      ...
      video.onerror = function() {
        console.log("Video kunne ikke afspilles:", fil.navn || fil.url);
        naesteSlideshowFil();
      };

      var afspil = video.play();
      if (afspil && typeof afspil.catch === "function") {
        afspil.catch(function() {
          console.log("Video blev afvist af browseren:", fil.navn || fil.url);
          naesteSlideshowFil();
        });
      }

      if (mobilVideo) {
        var mobilAfspil = mobilVideo.play();
        if (mobilAfspil && typeof mobilAfspil.catch === "function") {
          mobilAfspil.catch(function() {});
        }
      }

      slideshowFejlTimer = setTimeout(function() {
  ...
  crossfadeSlideshowBillede(fil);

  if (/\.gif$/i.test(fil.navn || fil.url)) {
    hentGifVarighed(fil.url)
      .then(function(varighedMs) {
        slideshowTimer = setTimeout(function() {
          naesteSlideshowFil();
        }, varighedMs);
      })
      .catch(function(fejl) {
        console.log("Kunne ikke aflæse GIF-varighed:", fejl);

        /* Fallback hvis GIF-varigheden ikke kan aflæses. */
        slideshowTimer = setTimeout(function() {
          naesteSlideshowFil();
        }, 5000);
      });
...
/***** STOR BILLEDFREMVISER PÅ PC *****/

var slideshowViewerIndex = 0;

function opsaetSlideshowViewer() {
  var slideshow = $("pcSlideshow");
  var viewer = $("slideshowViewer");
  if (!slideshow || !viewer) return;

  slideshow.addEventListener("click", function() {
    if (window.matchMedia && window.matchMedia("(max-width: 700px)").matches) return;
    aabnSlideshowViewer(slideshowIndex);
  });

  $("slideshowViewerLuk").addEventListener("click", lukSlideshowViewer);
  $("slideshowViewerForrige").addEventListener("click", function(event) {
    event.stopPropagation();
    skiftSlideshowViewer(-1);
  });
  $("slideshowViewerNaeste").addEventListener("click", function(event) {
    event.stopPropagation();
    skiftSlideshowViewer(1);
  });

  viewer.addEventListener("click", function(event) {
    if (event.target === viewer) lukSlideshowViewer();
  });

  document.addEventListener("keydown", function(event) {
    if (!viewer.classList.contains("vis")) return;

    if (event.key === "Escape") lukSlideshowViewer();
    if (event.key === "ArrowLeft") skiftSlideshowViewer(-1);
    if (event.key === "ArrowRight") skiftSlideshowViewer(1);
  });
}

function aabnSlideshowViewer(index) {
  var viewer = $("slideshowViewer");
  if (!viewer || slideshowBilleder.length === 0) return;

  slideshowViewerIndex = typeof index === "number" ? index : slideshowIndex;
  slideshowViewerIndex = Math.max(0, Math.min(slideshowViewerIndex, slideshowBilleder.length - 1));

  bygSlideshowViewerThumbnails();
  visSlideshowViewerFil();

  viewer.classList.add("vis");
  viewer.setAttribute("aria-hidden", "false");
  document.body.classList.add("slideshow-viewer-aaben");
}

function lukSlideshowViewer() {
  var viewer = $("slideshowViewer");
  var video = $("slideshowViewerVideo");
  if (!viewer) return;
  ...
  viewer.classList.remove("vis");
  viewer.setAttribute("aria-hidden", "true");
  document.body.classList.remove("slideshow-viewer-aaben");
}

function skiftSlideshowViewer(retning) {
  if (slideshowBilleder.length === 0) return;

  slideshowViewerIndex =
    (slideshowViewerIndex + retning + slideshowBilleder.length) % slideshowBilleder.length;

  visSlideshowViewerFil();
}

function visSlideshowViewerFil() {
  if (slideshowBilleder.length === 0) return;

  var fil = slideshowBilleder[slideshowViewerIndex];
  if (typeof fil === "string") fil = normaliserSlideshowListe([fil])[0];
  if (!fil || !fil.url) return;

  var img = $("slideshowViewerImg");
  var video = $("slideshowViewerVideo");
  var counter = $("slideshowViewerCounter");
  var navn = $("slideshowViewerNavn");

  if (counter) counter.textContent = (slideshowViewerIndex + 1) + " / " + slideshowBilleder.length;
  if (navn) navn.textContent = fil.navn || "";
  ...
  opdaterSlideshowViewerThumbnails();
}

function bygSlideshowViewerThumbnails() {
  var holder = $("slideshowViewerThumbnails");
  if (!holder) return;

  holder.innerHTML = "";

  slideshowBilleder.forEach(function(fil, index) {
    if (typeof fil === "string") fil = normaliserSlideshowListe([fil])[0];
    if (!fil || !fil.url) return;

    var knap = document.createElement("button");
    knap.type = "button";
    knap.className = "slideshow-viewer-thumb";
    knap.setAttribute("data-index", index);
    knap.title = fil.navn || ("Billede " + (index + 1));

    if (fil.type === "video") {
      var videoMarkering = document.createElement("span");
      videoMarkering.className = "slideshow-viewer-video-thumb";
      videoMarkering.textContent = "▶";
      knap.appendChild(videoMarkering);
    } else {
      var thumb = document.createElement("img");
      thumb.src = fil.url;
      thumb.alt = "";
      thumb.loading = "lazy";
      knap.appendChild(thumb);
    }

    knap.addEventListener("click", function(event) {
      event.stopPropagation();
      slideshowViewerIndex = index;
      visSlideshowViewerFil();
    });

    holder.appendChild(knap);
  });
}

function opdaterSlideshowViewerThumbnails() {
  var holder = $("slideshowViewerThumbnails");
  if (!holder) return;

  var knapper = holder.querySelectorAll(".slideshow-viewer-thumb");
  knapper.forEach(function(knap, index) {
    knap.classList.toggle("aktiv", index === slideshowViewerIndex);
    if (index === slideshowViewerIndex) {
      try { knap.scrollIntoView({ behavior: "smooth", block: "nearest", inline: "center" }); } catch (e) {}
    }
  });
}
