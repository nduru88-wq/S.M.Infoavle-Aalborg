...
function aabnRedigerSide(id, visningsDato) {
  ...
  // En allerede særskilt forekomst har sit eget ID og redigeres direkte.
  if (aktivitet.parentId) {
    gaaTilRedigering(id);
    return;
  }
  ...
  // VIGTIGT:
  // Før redigeringssiden åbnes, opretter/henter vi en rigtig særforekomst
  // med eget ID og Parent ID. Dermed kan redigeringen aldrig ramme
  // grundaktiviteten ved en fejl.
  google.script.run
    .withSuccessHandler(function(forekomstId) {
      if (!forekomstId) {
    ...
    .withFailureHandler(function(err) {
      alert("Kunne ikke åbne forekomsten: " + (err && err.message ? err.message : err));
    })
