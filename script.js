...
function tilpasMiniSpejl() {
  ...
  if (!box || !spejl || !hovedUge) return;

  /* Brug hovedtavlens faktiske størrelse som lærred, og skalér hele
     resultatet samlet ned. Dermed ændres proportionerne ikke. */
  var kildeBredde = hovedUge.clientWidth;
  var kildeHoejde = hovedUge.clientHeight;

  if (!kildeBredde || !kildeHoejde) return;

  spejl.style.width = kildeBredde + "px";
  spejl.style.height = kildeHoejde + "px";

  var sx = box.clientWidth / kildeBredde;
  var sy = box.clientHeight / kildeHoejde;
  var skala = Math.min(sx, sy);

  spejl.style.transform = "scale(" + skala + ")";
}
