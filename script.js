...
function sendTilTavle() {
  ...
  if (typeof AKTIVITET_ID !== "undefined" && AKTIVITET_ID && redigerForekomstDato) {
    // Gem kun DEN valgte forekomst som en særskilt række.
    // Grundaktiviteten og de øvrige gentagelser ændres ikke.
    runner.gemForekomst(
      AKTIVITET_ID,
      a.dato,
      a.person,
  ...
  } else if (typeof AKTIVITET_ID !== "undefined" && AKTIVITET_ID) {
...
function indlaesAktivitetTilRedigering(id) {
  ...
  google.script.run
    .withSuccessHandler(function(a) {
      ...
      // Ved en fremtidig forekomst af en gentagelse starter noten blank.
      // Hvis forekomsten allerede er gemt særskilt, åbnes dens eget ID i stedet
      // og dens egen note vises normalt.
      setVal("note", redigerForekomstDato ? "" : (a.note || ""));
...
function aabnRedigerSide(id, visningsDato) {
  ...
  // Kun en ikke-særskilt gentagelse på en anden dato end startdatoen
  // skal oprettes som individuel forekomst.
  if (aktivitet && !aktivitet.parentId && visningsDato &&
      String(visningsDato) !== String(aktivitet.dato)) {
    var g = String(aktivitet.gentagelse || "ingen").toLowerCase();
    if (g === "dagligt" || g === "ugentligt" || g === "14dage") {
      url += "&forekomstDato=" + encodeURIComponent(visningsDato);
    }
  }
