...
/***** ANJA ANNIVERSARY - BOUNCE + ON/OFF *****/

var anjaAnniversaryAktiv = true;
var anjaAnniversaryAnimationStartet = false;
var anjaAnniversaryX = 24;
var anjaAnniversaryY = 24;
var anjaAnniversaryVX = 95;
var anjaAnniversaryVY = 72;
var anjaAnniversarySidsteTid = null;

function opdaterAnjaAnniversaryKnap() {
  var knap = $("anjaAnniversaryToggle");
  if (!knap) return;
  knap.textContent = anjaAnniversaryAktiv ? "Anja: TIL" : "Anja: FRA";
  knap.classList.toggle("fra", !anjaAnniversaryAktiv);
  knap.setAttribute("aria-pressed", anjaAnniversaryAktiv ? "true" : "false");
}

function toggleAnjaAnniversary() {
  var box = $("anjaAnniversaryBounce");
  if (!box) return;
  anjaAnniversaryAktiv = !anjaAnniversaryAktiv;
  box.classList.toggle("aktiv", anjaAnniversaryAktiv);
  box.setAttribute("aria-hidden", anjaAnniversaryAktiv ? "false" : "true");
  opdaterAnjaAnniversaryKnap();
}

function startAnjaAnniversaryBounce() {
  var box = $("anjaAnniversaryBounce");
  var img = $("anjaAnniversaryImg");
  if (!box || !img || anjaAnniversaryAnimationStartet) return;

  anjaAnniversaryAnimationStartet = true;
  box.classList.toggle("aktiv", anjaAnniversaryAktiv);
  opdaterAnjaAnniversaryKnap();

  var harProevetLokal = false;
  img.addEventListener("error", function() {
    if (harProevetLokal) return;
    harProevetLokal = true;
    img.src = "slideshow/AnjaAnniversary.png?v=" + Date.now();
  });

  function holdIndenforSkaerm() {
    var maxX = Math.max(0, window.innerWidth - box.offsetWidth);
    var maxY = Math.max(0, window.innerHeight - box.offsetHeight);
    anjaAnniversaryX = Math.min(Math.max(0, anjaAnniversaryX), maxX);
    anjaAnniversaryY = Math.min(Math.max(0, anjaAnniversaryY), maxY);
  }

  function tegn(tid) {
    if (anjaAnniversarySidsteTid === null) anjaAnniversarySidsteTid = tid;
    var dt = Math.min((tid - anjaAnniversarySidsteTid) / 1000, 0.05);
    anjaAnniversarySidsteTid = tid;

    if (anjaAnniversaryAktiv) {
      var maxX = Math.max(0, window.innerWidth - box.offsetWidth);
      var maxY = Math.max(0, window.innerHeight - box.offsetHeight);

      anjaAnniversaryX += anjaAnniversaryVX * dt;
      anjaAnniversaryY += anjaAnniversaryVY * dt;

      if (anjaAnniversaryX <= 0) {
        anjaAnniversaryX = 0;
        anjaAnniversaryVX = Math.abs(anjaAnniversaryVX);
      } else if (anjaAnniversaryX >= maxX) {
        anjaAnniversaryX = maxX;
        anjaAnniversaryVX = -Math.abs(anjaAnniversaryVX);
      }

      if (anjaAnniversaryY <= 0) {
        anjaAnniversaryY = 0;
        anjaAnniversaryVY = Math.abs(anjaAnniversaryVY);
      } else if (anjaAnniversaryY >= maxY) {
        anjaAnniversaryY = maxY;
        anjaAnniversaryVY = -Math.abs(anjaAnniversaryVY);
      }

      box.style.transform = "translate3d(" + anjaAnniversaryX + "px," + anjaAnniversaryY + "px,0)";
    }

    requestAnimationFrame(tegn);
  }

  window.addEventListener("resize", holdIndenforSkaerm);
  holdIndenforSkaerm();
  requestAnimationFrame(tegn);
}
