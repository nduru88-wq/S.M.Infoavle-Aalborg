...
function indlaesAktivitetTilRedigering(id) {
  ...
  google.script.run
    .withSuccessHandler(function(a) {
      ...
      var redigerDato = redigerForekomstDato || a.dato;
      sikrDatoISelect("dato", redigerDato);
      setVal("dato", redigerDato);
  ...
  return String(a.tidspunkt || "").localeCompare(String(b.tidspunkt || ""));
    ...
    if (!dagens.length) {
      ...
      dagens.forEach(function(a) {
        var eventData = Object.assign({}, a);
        eventData._visningsDato = formatDatoInput(dato);
        dagScroll.appendChild(lavEventElement(eventData));
      });
    }
...
function startAutomatiskDagScroll() {
  ...
  /* Både hovedtavlen og spejlet af næste uge bruger samme automatiske scroll.
     Hver dagskolonne scroller kun, hvis dens egne aktiviteter fylder for meget. */

  var felter = document.querySelectorAll("#week .day-scroll, #miniNaesteUge .day-scroll");
...
if (erFoedselsdag) {
  ...
  "<div class='event-main-content'>" +
    "<div class='event-title'><span class='event-title-name'>" + titel + (tid ? ":" : "") + "</span>" + (tid ? "<span class='event-title-time'>" + tid + "</span>" : "") + "</div>" +
    lavEventNote(a) +
  "</div>" +
 ...
 "<div class='event-actions'>" +
  ...
  "<button class='edit-btn' onclick=\"event.stopPropagation(); aabnRedigerSide('" + id + "', '" + escapeHtml(a._visningsDato || a.dato || "") + "')\">Ret</button>" +
"</div>";
...
function lavEventNote(a) {
  ...
  var erGentaget = gentagelse === "dagligt" ||
                   gentagelse === "ugentligt" ||
                   gentagelse === "14dage";

  // Grundaktivitetens note gælder kun startdatoen.
  // En særskilt forekomst (parentId) viser altid sin egen note.
  if (erGentaget && !a.parentId && a._visningsDato &&
      String(a._visningsDato) !== String(a.dato)) {
    note = "";
  }

  return note
    ? "<div class='event-note'>" + escapeHtml(note) + "</div>"
    : "";
}
...
function visPersonale() {
  ...
  function lavPersonElement(person) {
    ...
    var label = "";
    var statusFarve = "green";

    /* Heldagsfravær / røde statustyper */
    if (fravaer) {
      ...
      /* Optaget i huset = gul i selve tidsrummet */
      if (label === "Møde" || label === "Faglig sparring") {
        label = "Møde";
        statusFarve = "yellow";
      }
      /* Disse statustyper skal være røde i selve tidsrummet */
      else if (label === "Møder senere" || label === "Ude af huset") {
        if (label === "Ude af huset") {
          label = "Ude";
        }
        statusFarve = "red";
    ...
    div.className =
      "person person-" + statusFarve +
      (label ? " away" : "");

    div.innerHTML =
      "<div class='person-name'>" + escapeHtml(person) + "</div>" +
...
function statusSkalVisesNu(a) {
  ...
  if (
    a.aktivitet === "Møde" ||
    a.aktivitet === "Faglig sparring" ||
    a.aktivitet === "Ude af huset"
  ) {
...
function aabnRedigerSide(id, visningsDato) {
  if (!id) {
    alert("Mangler ID på aktiviteten.");
    return;
  }

  // Giv øjeblikkelig feedback, mens en evt. særforekomst oprettes/hentes.
  var trykketRetKnap = null;
  try {
    trykketRetKnap = event && event.currentTarget ? event.currentTarget : null;
  } catch (e) {}

  if (trykketRetKnap) {
    trykketRetKnap.disabled = true;
    trykketRetKnap.dataset.originalText = trykketRetKnap.textContent;
    trykketRetKnap.textContent = "Åbner…";
    trykketRetKnap.classList.add("loading");
  }

  function nulstilRetKnap() {
    if (!trykketRetKnap) return;
    trykketRetKnap.disabled = false;
    trykketRetKnap.textContent = trykketRetKnap.dataset.originalText || "Ret";
    trykketRetKnap.classList.remove("loading");
  }

  var aktivitet = aktiviteterGlobal.find(function(a) {
    return String(a.id) === String(id);
  });

  function gaaTilRedigering(redigerId) {
    window.location.href = "opret.html?id=" + encodeURIComponent(redigerId);
  }

  if (!aktivitet || aktivitet.parentId) {
    gaaTilRedigering(id);
    return;
  }

  var g = String(aktivitet.gentagelse || "ingen").toLowerCase();
  var erGentaget = g === "dagligt" || g === "ugentligt" || g === "14dage";
  var erSenereForekomst =
    erGentaget &&
    visningsDato &&
    String(visningsDato) !== String(aktivitet.dato);

  // Første forekomst eller almindelig aktivitet redigeres som før.
  if (!erSenereForekomst) {
    gaaTilRedigering(id);
    return;
  }

  function aabnSaerforekomst(forekomstId) {
    if (!forekomstId) {
      nulstilRetKnap();
      alert("Kunne ikke oprette den valgte forekomst til redigering.");
      return;
    }
    gaaTilRedigering(forekomstId);
  }

  function visRedigerFejl(err) {
    nulstilRetKnap();
    alert(
      "Kunne ikke åbne forekomsten: " +
      (err && err.message ? err.message : String(err || "Ukendt fejl"))
    );
  }

  // Hvis tavlen kører inde i Apps Script, brug google.script.run.
  if (typeof google !== "undefined" && google.script && google.script.run) {
    google.script.run
      .withSuccessHandler(aabnSaerforekomst)
      .withFailureHandler(visRedigerFejl)
      .hentEllerOpretForekomst(id, visningsDato);
    return;
  }

  // GitHub/almindelig webside: brug samme JSONP-princip som resten af tavlen.
  var callbackName = "smForekomstCallback_" + Date.now();
      ...
      if (!data || data.ok === false || !data.id) {
        visRedigerFejl({
          message: data && data.message
            ? data.message
            : "Apps Script returnerede ikke et forekomst-ID."
        });
        return;
      }
      aabnSaerforekomst(data.id);
    } finally {
      delete window[callbackName];
      jsonpScript.remove();
    }
  ...
  jsonpScript.onerror = function() {
    delete window[callbackName];
    jsonpScript.remove();
    visRedigerFejl({ message: "Kunne ikke kontakte Apps Script." });
  };

  jsonpScript.src =
    "https://script.google.com/macros/s/AKfycbySJ7n3fUmVg4dtVDKxhfYsIhh8S5REFfCeoT9F1H8g5fY0PSGBEtx4Y95vF_8Htxrh/exec" +
    "?action=hentEllerOpretForekomst" +
    "&parentId=" + encodeURIComponent(id) +
    "&dato=" + encodeURIComponent(visningsDato) +
    "&token=" + encodeURIComponent(hentAdminTokenFraBrowser()) +
    "&callback=" + encodeURIComponent(callbackName);

  jsonpScript.src = jsonpScript.src;
...
/***** MINI NÆSTE UGE – 1:1 SPEJL AF UGETAVLEN *****/

function visMiniNaesteUge() {
  var box = $("miniNaesteUge");
  var hovedUge = $("week");
  if (!box || !hovedUge || mobilVisning) return;

  box.innerHTML = "";

  /*
    Mini-tavlen bruger nu PRÆCIS de samme .day-, .day-scroll- og .event-
    elementer som hovedtavlen. Der findes derfor ikke længere en separat
    mini-event-rendering, som kan komme ud af takt med hovedtavlen.
  */
  var spejl = document.createElement("div");
  spejl.className = "week mini-mirror-week";
  ...
  for (var i = 0; i < 5; i++) {
    ...
    day.appendChild(dagScroll);
    spejl.appendChild(day);
  }

  box.appendChild(spejl);
...
function tilpasMiniSpejl() {
  var box = $("miniNaesteUge");
  var spejl = box ? box.querySelector(".mini-mirror-week") : null;
  var kilde = $("week");

  if (!box || !spejl || !kilde) return;

  var kildeBredde = kilde.clientWidth;
  var kildeHoejde = kilde.clientHeight;

  if (!kildeBredde || !kildeHoejde || !box.clientWidth || !box.clientHeight) return;

  /* Først skaleres tavlen proportionalt efter bredden.
     Derefter får højden lov til at bruge næsten hele den ledige plads.
     Det giver en lidt større/læsbar mini-tavle uden at ændre klon- eller scroll-logikken. */
  var scaleX = box.clientWidth / kildeBredde;
  var proportionalHoejde = kildeHoejde * scaleX;
  var ledigHoejde = box.clientHeight;

  /* Mild lodret strækning: højst 8 %, og aldrig ud over boksen. */
  var scaleY = scaleX;
  if (proportionalHoejde < ledigHoejde) {
    scaleY = Math.min(
      ledigHoejde / kildeHoejde,
      scaleX * 1.08
    );
  }

  spejl.style.width = kildeBredde + "px";
  spejl.style.height = kildeHoejde + "px";
  spejl.style.transform = "scale(" + scaleX + ", " + scaleY + ")";

  /* Centrer tavlen lodret i den tilgængelige plads. */
  var vistHoejde = kildeHoejde * scaleY;
  spejl.style.top = Math.max(0, (ledigHoejde - vistHoejde) / 2) + "px";
}

window.addEventListener("resize", function() {
  requestAnimationFrame(tilpasMiniSpejl);
});
