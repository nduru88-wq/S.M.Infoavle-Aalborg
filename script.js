var API_URL = "https://script.google.com/macros/s/AKfycbwqFQBUoXySQQ5kdM9FnhqbMu5X97160BNtn-ZQLwZB1QDzb9XtvwKfoaHZru2ioKFP/exec";
...
var mobilTouchSidsteX = null;
var mobilTouchStartTid = 0;
var mobilSwipeAktiv = false;
var mobilSwipeVandret = null;
...
function mobilSwipePosition(forskelX) {
  var week = $("week");
  if (!week) return;

  week.style.transition = "none";
  week.style.transform = "translateX(calc(-100vw + " + forskelX + "px))";
}

function mobilSwipeTil(position, faerdig) {
  var week = $("week");
  if (!week) return;

  week.style.transition = "transform 220ms cubic-bezier(.22,.61,.36,1)";
  week.style.transform = "translateX(" + position + ")";
...
function nulstilMobilSwipe() {
  mobilTouchStartX = null;
  mobilTouchStartY = null;
  mobilTouchSidsteX = null;
  mobilTouchStartTid = 0;
  mobilSwipeAktiv = false;
  mobilSwipeVandret = null;
}
...
function opsaetMobilSwipe() {
  document.addEventListener("touchstart", function(event) {
    ...
    mobilTouchSidsteX = mobilTouchStartX;
    mobilTouchStartTid = Date.now();
    mobilSwipeAktiv = true;
    mobilSwipeVandret = null;
  ...
  document.addEventListener("touchmove", function(event) {
    ...
    var forskelX = x - mobilTouchStartX;
    var forskelY = y - mobilTouchStartY;

    if (mobilSwipeVandret === null) {
      if (Math.abs(forskelX) < 7 && Math.abs(forskelY) < 7) {
        return;
      }

      mobilSwipeVandret = Math.abs(forskelX) > Math.abs(forskelY);
    }

    if (!mobilSwipeVandret) {
      return;
    }

    event.preventDefault();
    mobilTouchSidsteX = x;
    mobilSwipePosition(forskelX);
  }, { passive: false });
  ...
  document.addEventListener("touchend", function(event) {
    ...
    var slutX = mobilTouchSidsteX !== null
      ? mobilTouchSidsteX
      : (
    ...
    var varighed = Math.max(Date.now() - mobilTouchStartTid, 1);
    var hastighed = Math.abs(forskelX) / varighed;
    var bredde = window.innerWidth || 360;

    var skalSkifte =
      mobilSwipeVandret &&
      (
        Math.abs(forskelX) > bredde * 0.24 ||
        (Math.abs(forskelX) > 55 && hastighed > 0.45)
      );

    var retning = forskelX < 0 ? 1 : -1;
  ...
  document.addEventListener("touchcancel", function() {
    if (!mobilVisning || !mobilSwipeAktiv) return;
...
function visUge() {
  ...
  if (mobilVisning) {
    ...
    start.setDate(start.getDate() + mobilDagOffset - 1);
    antalDage = 3;
